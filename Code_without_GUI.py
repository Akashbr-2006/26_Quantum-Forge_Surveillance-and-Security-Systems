...
class HomePage(tk.Frame):
    ...
    def process_frame(self):
        ...
        if self.phone_cam.isOpened():
            ...
            if ret1:
                ...
                # Convert the frame to RGB (Tkinter only supports RGB images)
                phone_frame_rgb = cv2.cvtColor(phone_frame, cv2.COLOR_BGR2RGB)
                phone_frame_pil = Image.fromarray(phone_frame_rgb)
                phone_frame_tk = ImageTk.PhotoImage(phone_frame_pil)

                # Update the canvas with the new frame
                self.canvas.create_image(0, 0, anchor="nw", image=phone_frame_tk)
                self.canvas.image = phone_frame_tk  # Keep a reference to avoid garbage collection
    ...
    def count_people_in_grid(self, detections, frame_width, frame_height, grid_size=(3, 3)):
    ...
    def draw_grid_and_counts(self, frame, grid_counts, grid_size=(3, 3)):
        ...
        for i in range(grid_size[0]):
            for j in range(grid_size[1]):
                ...
                x = j * cell_width + 10
                y = i * cell_height + 30
                cv2.putText(frame, count_text, (x, y), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
